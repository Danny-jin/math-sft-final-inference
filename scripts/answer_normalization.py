...
# ---------------------------------------------------------------------------
# Box extraction / replacement — proper brace counter (regex can't do depth>1)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Compound-symbolic detector + decimal converter
# Judger's parse_latex FAILS on these — they must be evaluated to decimal
# regardless of mathematical correctness:
#   \arctan, \arcsin, \arccos, e, e^x, \frac{\sqrt{X}}{Y}, \frac{\ln(a)}{\ln(b)}
# ---------------------------------------------------------------------------
_UNSAFE_PATTERNS = [
    re.compile(r'\\arctan\b'),
    re.compile(r'\\arcsin\b'),
    re.compile(r'\\arccos\b'),
    re.compile(r'(?<!\\)\be\b(?!\w)'),                       # bare e
    re.compile(r'(?<!\\)\be\^'),                              # e^...
    re.compile(r'\\(?:d|t)?frac\{[^{}]*\\sqrt'),              # \frac{...\sqrt...}{...}
    re.compile(r'\\(?:d|t)?frac\{\\ln'),                      # \frac{\ln(a)}{\ln(b)}
]


def _is_unsafe_symbolic(s: str) -> bool:
    return any(p.search(s) for p in _UNSAFE_PATTERNS)


def _eval_to_decimal(s: str, sig_figs: int = 12) -> str | None:
    """
    Try to numerically evaluate a LaTeX expression to a ≥12 sig fig decimal.
    Returns the decimal string, or None if evaluation fails.
    """
    s = s.strip()
    ...
    try:
        ...
        from sympy import N
        v = float(N(parse_latex(s), sig_figs + 3))
        return f'{v:.{sig_figs}g}'
    except Exception:
        pass
    # Try regex LaTeX→Python translation + sympify
    try:
        from sympy import sympify, N
        py = _latex_to_sympy_str(s)
        v = float(N(sympify(py), sig_figs + 3))
        return f'{v:.{sig_figs}g}'
    except Exception:
        return None
...
def _latex_to_sympy_str(s: str) -> str:
    """Same translator as in scripts/lenient_judge.py — for self-contained use."""
    s = str(s).strip()
    s = s.replace(r'\,', '').replace(r'\!', '').replace(r'\;', '').replace(r'\:', '')
    s = re.sub(r'\\\\\s*', ' ', s)
    s = s.replace(r'\pi', 'pi').replace(r'\infty', 'oo')
    # standalone Euler's e → sympy's E (uppercase). Match 'e' not next to letters.
    s = re.sub(r'(?<![a-zA-Z\\])e(?![a-zA-Z])', 'E', s)
    for prefix in (r'\dfrac', r'\tfrac', r'\frac'):
        s = re.sub(re.escape(prefix) + r'\{([^{}]+)\}\{([^{}]+)\}',
                   r'((\1)/(\2))', s)
    s = re.sub(r'\\sqrt\{([^{}]+)\}', r'sqrt(\1)', s)
    for fn in ('ln', 'log', 'sin', 'cos', 'tan', 'arctan', 'arcsin',
               'arccos', 'exp'):
        s = s.replace('\\' + fn, fn)
...
def normalize_pred(pred: str, gold_list: list) -> str:
    ...
    # 4. UNSAFE compound symbolic → decimal (parse_latex would fail)
    #    e.g. \arctan(1/8) → 0.124354994546
    converted = []
    for p in parts:
        if _is_unsafe_symbolic(p):
            d = _eval_to_decimal(p)
            converted.append(d if d is not None else p)
        else:
            converted.append(p)
    parts = converted
    # 5. Re-wrap tuples if gold structure asks
    parts = maybe_rewrap_tuple(parts, gold_list)
    # 6. Precision match: truncate each numeric pred-part to gold's precision
    if len(parts) == len(gold_list):
        parts = [match_precision(p, str(g)) for p, g in zip(parts, gold_list)]
    # 7. Rejoin with canonical separator
    new_box = ', '.join(parts)
...
def normalize_pred_no_gold(pred: str) -> str:
    ...
    parts = split_top_level(box)
    parts = [clean_text(p) for p in parts]
    # UNSAFE compound symbolic → decimal
    out_parts = []
    for p in parts:
        if _is_unsafe_symbolic(p):
            d = _eval_to_decimal(p)
            out_parts.append(d if d is not None else p)
        else:
            out_parts.append(p)
    new_box = ', '.join(out_parts)
    return replace_box(pred, new_box)
